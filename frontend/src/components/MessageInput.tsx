...
const MessageInput: React.FC<MessageInputProps> = ({
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 pt-0 flex-shrink-0">
      <div
        id="drop-zone"
        className="glass panel-surface rounded-xl p-4"
        style={{ backgroundColor: "var(--chat-input-background-color)" }}
      >
        {image && (
          ...
          <div className="flex items-center ml-4">
            {onCaptureScreenshot && (
              <button
                ...
                className="mr-3 px-3 py-2 rounded-lg text-white font-semibold bg-gray-700 hover:bg-gray-600 transition-colors duration-200"
              >
                Capture
              </button>
            )}
            ...
            {isStreaming ? (
              ...
              <button
                id="send-button"
                className="ml-4 bg-blue-500 hover:bg-blue-600 text-white font-bold p-2 rounded-full w-10 h-10 flex items-center justify-center flex-shrink-0"
                onClick={handleSend}
              >
                <svg
